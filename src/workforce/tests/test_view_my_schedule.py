...
class TestWorkerSchedule(TestCase):
    ...
    def test_considering_worker_when_negative_offsets_upper_limit(self):
        ...
        created_event = create_event_at(
            time(23, 30), self.from_sao_paulo, self.user_without_photo)
        retrieved_events = _get_events_from_date(
            self.from_sao_paulo, datetime.now())
        self.assertCountEqual(to_comparable(
            retrieved_events), to_comparable([created_event]))

    def test_considering_worker_when_negative_offsets_lower_limit(self):
        ''' the Apr 1st for someone that lives in a -3 offset goes from Mar 31st 21:00 to Apr 1st 20:59 '''
        created_event = create_event_at(
            time(0, 0), self.from_sao_paulo, self.user_without_photo)
        retrieved_events = _get_events_from_date(
            self.from_sao_paulo, datetime.now())
        self.assertCountEqual(to_comparable(
            retrieved_events), to_comparable([created_event]))
    ...
    def test_considering_worker_when_positive_offsets_upper_limit(self):
        ...
        '''
        created_event = create_event_at(
            time(23, 30), self.from_japan, self.user_without_photo)
        retrieved_events = _get_events_from_date(
            self.from_japan, datetime.now())
        self.assertCountEqual(to_comparable(
            retrieved_events), to_comparable([created_event]))
    ...
    def test_considering_worker_when_positive_offsets_lower_limit(self):
        ...
        '''
        created_event = create_event_at(
            time(0, 0), self.from_japan, self.user_without_photo)
        retrieved_events = _get_events_from_date(
            self.from_japan, datetime.now())
        self.assertCountEqual(to_comparable(
            retrieved_events), to_comparable([created_event]))
    ...
    def test_can_show_schedule_with_events_without_photo(self):
        create_event_at(time(3, 0), self.from_sao_paulo,
                        self.user_without_photo)
        create_event_at(time(7, 0), self.from_sao_paulo,
                        self.user_without_photo)
        ...
        self.assertTrue(
            len(response.context['today_events']) == 2, 'Number of events differs')
...
def create_event_at(hour, worker, user):
    ...
    start = arrow.get(datetime.combine(today_for_worker, hour)
                      ).replace(tzinfo=worker.timezone)
